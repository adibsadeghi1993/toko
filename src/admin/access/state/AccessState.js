...
const AccessState = ({ children }) => {
  ...
  const promoter_level = async () => {
    ...
    let res = await FetchRequest(
      "http://81.91.145.250:8002/outsource/promoter_level/",
      {},
      "GET"
    );
    if (res) {
  ...
  const MemmberLevel = async (id_level) => {
    ...
    let res = await FetchRequest(
      "http://81.91.145.250:8002/outsource/parent_level_members/",
  ...
  const PercentsDefault = async (id_member) => {
    ...
    try {
      let res = await FetchRequest(
        "http://81.91.145.250:8002/outsource/sale_percents_default/",
        {
  ...
  const updatePercent = async () => {
    ...
    Object.keys(state.percents).map((key, inx) => {
      return state.percents[key].map((item, ind) => {
