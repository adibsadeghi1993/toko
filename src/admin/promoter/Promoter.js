...
import PromoterableRow from "./controls/PromoterableRow";
// import MemmberRoles from "./controls/MemmberRoles";
// import MemmberTableRow from "./controls/MemmberTableRow";
import { MemmberContext, PromoterContext } from "./state/State";
import Top from "./Top";

export default React.memo(() => {
  const { getMemmbers, memmbers, loading, role_id, dispatch } =
    useContext(PromoterContext);
            ...
            <div className="relative md:flex md:justify-center mt-5 overflow-x-scroll lg:overflow-x-auto">
              <table className="md:w-11/12">
                <thead className="text-sm bg-gray-300">
                  <tr>
                    <th className="whitespace-nowrap px-4 py-2 w-full">
                      نام کاربر
                    </th>
                ...
                <tbody>
                  {!!memmbers &&
                    memmbers?.result?.map((item, index) => (
                      <PromoterableRow key={index} item={item} />
                    ))}
                </tbody>
              </table>
            </div>
