                    ...
                    <tbody>
                        {
                            person_info?.map((person) =>
                                ...
                                <tr className="bg-emerald-200 text-center hover:text-blue-500 cursor-pointer"
                                    onClick={() => {
                                        dispatch({ type: "SET_REAL", payload: `اطلاعات ${person?.full_name} - ${person?.relation === "Insurer" ? "بیمه گذار" : person?.relation}` })
                                        //    در صورت نمایش دیتای بیمه گذار
                                        if (person?.relation === "Insurer") {
                                            dispatch({ type: "SET_INSURER_TREATMENT", payload: true })
                                        }
                                        dispatch({ type: "SET_STEP", payload: STEP_SALE_TAB.DETAILS })
                                    }}>
                                    <td className='py-2 border border-gray-300'>{person?.full_name}</td>
                                    <td className='py-2 border border-gray-300'>{person?.father_name || "-"}</td>
                                    <td className='py-2 border border-gray-300'>{person?.relation || "-"}</td>
                                    <td className='py-2 border border-gray-300'>{person?.fran || "-"}</td>
                                    <td className='py-2 border border-gray-300'> {(moment(person?.birthday, "YYYY-MM-DD").isValid() &&
                                        moment(person?.birthday, "YYYY-MM-DD")
                                            .endOf("jMonth")
                                            .format("jYYYY/jM/jD")) ||
                                        person?.birthday}</td>
                                    <td className='py-2 border border-gray-300'>{person?.gender || "-"}</td>
                                    <td className='py-2 border border-gray-300'>{person?.base_ins || "-"}</td>
                                    <td className='py-2 border border-gray-300'>{person?.life_company || "-"}</td>
