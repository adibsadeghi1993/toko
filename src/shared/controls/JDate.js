...
class JDate {
  ...
  startOfJMonth() {
    ...
    const { gy, gm, gd } = dateShamsi
      ? jalaali.toGregorian(dateShamsi.jy, dateShamsi.jm, 1)
      : {};
  ...
  jMonthLength() {
    let dateShamsi = this._date && jalaali.toJalaali(this._date);
    return (
      dateShamsi && jalaali.jalaaliMonthLength(dateShamsi.jy, dateShamsi.jm)
    );
  }
  ...
  getjDatetimeStr() {
    ...
    return `${new Date(this._date).getHours()}:${new Date(
      this._date
    ).getMinutes()} ${jy}/${jm}/${jd}`;
  }
  getjDateStr(sp = "-") {
  ...
  getgDatetimeStr() {
    ...
    ).slice(-2)}-${("0" + this._date.getDate()).slice(-2)}`;
  }
}
